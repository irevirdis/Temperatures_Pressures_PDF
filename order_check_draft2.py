...
myPolyint = Polyint([uni_1, uni_2], myBasis)

# dummy function to test computeCoefficients
def dummy(x): 
    return 2*x**2

#----------------------------------------------------------------#
# part 1: method computeCoefficients call

coeff = myPolyint.computeCoefficients(dummy)
...
#----------------------------------------------------------------#
def efficiency(t1,t2,p1,p4):
    gamma =1.4
    eta = (t1 -t2)/(t1* (1- (p2/p1)**((gamma-1.)/gamma)))
    return eta

def check_order(fnc):
    coefficients = myPolyint.computeCoefficients(efficiency)
    return coefficients
