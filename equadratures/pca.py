...
class Pca(object):
    """ The class defines a Principal Component Analysis.
    ...
    def C2U(self, X):
        ...
        for j in range(len(self.D)):
            for i in range(len(X)):
                #cdf = self.D[j].getCDF(points=X[i,j])
                #y[i,j] = self.std.getiCDF(cdf)
                y [i,j] = (X[i,j] - self.D[j].mean)/np.sqrt(self.D[j].variance)
                #y[i,j] = X[i,j]
